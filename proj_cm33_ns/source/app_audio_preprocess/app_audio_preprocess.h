...
* 6. 按真实模型要求输出 float32[40,T]，并只把处理好的模型输入写入共享内存。
*
...
#define APP_AUDIO_SPECTRUM_COMPARE_WINDOWS                           (3u)
     ...
     (APP_AUDIO_MODEL_SELECT == APP_AUDIO_MODEL_SELECT_3W_E2_PEAK_PREVIEW) || \
     (APP_AUDIO_MODEL_SELECT == APP_AUDIO_MODEL_SELECT_HZ2_0_B0_CLEANLINE))
...
#error "selector 4 requires APP_AUDIO_PREPROCESS_FRONTEND_PROFILE_BOARD_HTK_NO_NORM_V1"
     ...
     (APP_AUDIO_PREPROCESS_DEFAULT_FRONTEND_PROFILE != \
      APP_AUDIO_PREPROCESS_FRONTEND_PROFILE_BOARD_HTK_NO_NORM_V1))
#error "selector 7 requires APP_AUDIO_PREPROCESS_FRONTEND_PROFILE_BOARD_HTK_NO_NORM_V1"
#endif
